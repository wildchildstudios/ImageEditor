...
export function GroupEditPanel() {
    ...
    const selectedElement = useMemo(() => {
        ...
        return el?.type === 'group' ? el as GroupElement : null;
    ...
    const childCount = selectedElement?.children?.length || 0;

    return (
        <div className="w-64 h-full flex flex-col bg-white border-l border-gray-200">
            {/* Header */}
            <div className="p-4 border-b border-gray-200">
                <h2 className="text-gray-800 font-semibold text-sm">Edit Group</h2>
                {selectedElement && (
                    <p className="text-[10px] text-gray-400 mt-0.5">{childCount} elements</p>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {/* Ungroup Section */}
                <div className="p-4">
                    <h4 className="text-xs font-medium text-gray-500 mb-3">Group Actions</h4>
                    <div className="grid grid-cols-2 gap-1">
                        <button
                            onClick={() => selectedElement && ungroupElement(selectedElement.id)}
                            disabled={isDisabled}
                        ...
                        >
                            <Ungroup size={16} />
                            <span className="text-[9px] mt-1 font-medium leading-none">Ungroup</span>
                        </button>

                        <button
                            onClick={() => selectedElement && (isLocked ? unlockElement(selectedElement.id) : lockElement(selectedElement.id))}
                            disabled={isDisabled}
                            className={`flex flex-col items-center text-center justify-center py-2 px-0 rounded-lg transition-all
                                ${!isDisabled
                                    ? isLocked
                        ...
                        >
                            ...
                            <span className="text-[9px] mt-1 font-medium leading-none">{isLocked ? 'Unlock' : 'Lock'}</span>
                        </button>
                    </div>
                </div>
